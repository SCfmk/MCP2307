...
namespace MCP23017 {
  ...
  function writeReg(addr: number, reg: number, val: number) {
    // Write: [reg, val] using 16-bit BE helper
    const word = ((reg & 0xFF) << 8) | (val & 0xFF)
    pins.i2cWriteNumber(addr, word, NumberFormat.UInt16BE, false)
  }
  ...
  export function pinMode(pin: number, mode: Mode) {
    ...
    if (mode == Mode.INPUT) dir |= (1 << idx) else dir &= ~(1 << idx)
